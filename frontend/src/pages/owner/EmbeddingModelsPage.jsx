...
const EmbeddingModelsPage = () => {
  ...
  const handleDelete = async (row) => {
    ...
    try {
      await embeddingModelsAPI.delete(row.id);
      refresh();
    } catch (e) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-semibold text-ink">Embedding Models</h1>
        <button
          className={buttonClass}
          onClick={() => navigate('/owner/ai-providers/embeddings/new')}
        >
          + Add new Embedding
        </button>
      </div>
      ...
      {!loading && rows.length === 0 && (
        <div className="border border-dashed border-ink/20 rounded-sm p-8 text-center">
          <p className="text-ink/70">No embedding models configured yet.</p>
          <Link
            to="/owner/ai-providers/embeddings/new"
            ...
            {rows.map((row) => (
              <tr key={row.id} className="border-t border-ink/10">
                ...
                <td className="px-3 py-2 text-right space-x-2">
                  <Link
                    to={`/owner/ai-providers/embeddings/${row.id}`}
