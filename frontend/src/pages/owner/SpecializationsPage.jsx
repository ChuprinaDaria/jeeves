...
const SpecializationsPage = () => {
  ...
  const handleDelete = async (row) => {
    ...
    try {
      await specializationsAPI.delete(row.id);
      refresh();
    } catch (e) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-semibold text-ink">Specializations</h1>
        <button
          className={buttonClass}
          onClick={() => navigate('/owner/specializations/new')}
        >
          + Add Specialization
        </button>
      </div>
      ...
      {!loading && rows.length === 0 && (
        <div className="border border-dashed border-ink/20 rounded-sm p-8 text-center">
          <p className="text-ink/70">No specializations configured yet.</p>
          <Link
            to="/owner/specializations/new"
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.id} className="border-t border-ink/10">
                ...
                <td className="px-3 py-2 text-xs">{row.branch?.name ?? '—'}</td>
                ...
                <td className="px-3 py-2 text-right space-x-2">
                  <Link
                    to={`/owner/specializations/${row.id}`}
