...
from Jeeves.clients.models import ClientAPIKey, Client, ClientDocument
    ...
    def post(self, request):
        ...
        try:
            ...
            if not language and message:
                try:
                    from Jeeves.clients.tasks import detect_language_from_messages
            ...
            if hasattr(rag_response, 'lead_data') and rag_response.lead_data and conv:
                try:
                    from Jeeves.clients.services.lead_extraction import save_lead_from_extraction
        ...
        try:
            ...
            try:
                ...
                if not existing_api_key:
                    ...
                    from Jeeves.clients.models import generate_api_key
...
class EmbeddingModelReindexView(APIView):
    ...
    def post(self, request, model_id):
        ...
        from Jeeves.clients.models import ClientEmbedding, ClientDocument
        ...
        from Jeeves.EmbeddingModel.tasks import reindex_documents_for_model
        model_pk = getattr(model, 'pk', None) or getattr(model, 'id', None)
        if model_pk is None:
...
class ClientIndexNewDocumentsView(APIView):
    ...
    def post(self, request):
        ...
        from Jeeves.EmbeddingModel.tasks import index_new_client_documents_task
...
class ClientReindexDocumentsView(APIView):
    ...
    def post(self, request):
        ...
        from Jeeves.EmbeddingModel.tasks import reindex_client_documents_task
        client_pk = getattr(client, 'pk', None) or getattr(client, 'id', None)
        if client_pk is None:
            return Response({'error': 'Invalid client ID'}, status=status.HTTP_400_BAD_REQUEST)
...
class LLMProvidersListView(APIView):
    ...
    def get(self, request):
        ...
        if client:
            from Jeeves.EmbeddingModel.models import LLMProvider as _LP
...
class ModelPairsView(APIView):
    ...
    def get(self, request):
        from Jeeves.clients.views import get_client_from_request
        from Jeeves.EmbeddingModel.models import LLMProvider, EmbeddingModel
...
class SaveSandboxQAView(APIView):
    ...
    def post(self, request):
        from Jeeves.clients.views import get_client_from_request
        from Jeeves.clients.models import KnowledgeBlock, ClientDocument
...
class PackageReceiveView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            from Jeeves.clients.package_sync import _get_client_type_from_package_type
