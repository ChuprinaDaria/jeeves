...
class TelegramWebhookView(View):
    ...
    def handle_regular_message(self, chat_id: int, message_text: str, username: str, first_name: str, client_hint=None):
        ...
        try:
            ...
            from Jeeves.clients.auto_reply import should_consultant_respond
    ...
    def handle_manager_reply(self, message, reply_to, client_hint):
        ...
        try:
            ...
            from Jeeves.clients.tasks import process_manager_hitl_response
            ...
            if callback_data.startswith('rate_'):
                ...
                if len(parts) == 3:
                    ...
                    try:
                        ...
                        if rating == 'negative':
                            from Jeeves.clients.tasks import close_session_and_send_email
                            try:
        ...
        try:
            ...
            if getattr(conversation, 'is_waiting_for_manager', False):
                ...
                if escalation_started and (timezone.now() - escalation_started) > timedelta(minutes=timeout_minutes):
                    ...
                    # Continue to generate normal response below with timeout notice
                else:
                    # Still within timeout - add a note but continue with normal response
                    # This allows the user to continue chatting while waiting
                    waiting_notes = {
            ...
            logger.info(f"Using language '{language}' for conversation {conversation.id} (message: '{message_body[:30]}...', last_user_language: {getattr(conversation, 'last_user_language', 'N/A')})")
    ...
    def _get_bot_token_for_chat(self, chat_id: int, client_hint=None) -> str:
        ...
        """
        try:
            from Jeeves.clients.models import ClientWhatsAppConversation, Client
